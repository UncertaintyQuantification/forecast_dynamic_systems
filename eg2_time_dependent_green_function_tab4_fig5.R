...
for(i in 1:length(plot_index_set)){
  ...
  plot_list[[as.character(plot_index)]] =   ggplot() +
    ...
    scale_y_continuous(breaks=seq(min(ppgp_ci$lower), max(ppgp_ci$upper), length.out=4),
                       labels = scientific_10) + 
    scale_linetype_manual(values = c("solid","dashed","dashed", "dashed", "dashed")) +
    scale_color_manual(values=c("black","orange", "#800020", "#FACC5F", "#4169E1")) + 
...
### zoom-in plots
zoom_in_plot_list = list()
for(plot_index in plot_index_set){
  truth = output_all_r[plot_index,(n_training+1):(n_training+num_plot)]
  ar1_pred = pred.fgasp.exp[plot_index, 1:num_plot]
  dmd_pred = out_sample_pred_dmd_r[plot_index,1:num_plot]
  hodmd_pred = out_sample_pred_hodmd[plot_index,1:num_plot]
  #ppgp_pred = c(rep(NA, n_training), m_predict_record_median_r[1:2000, plot_index])
  ppgp_pred =m_predict_record_r[1:num_plot, plot_index]
  
  plot1_df <- data.frame(step=rep((n_training+1):(n_training+num_plot), 5),
                         method = factor(rep(c("Truth","AR(1)","DMD", "HODMD", "PP-GP"), each=(num_plot)),levels=c("Truth","AR(1)","DMD", "HODMD", "PP-GP")),
                         pred = c(truth, ar1_pred,dmd_pred, hodmd_pred, ppgp_pred))
  zoom_in_plot_list[[as.character(plot_index)]] = ggplot() +
    geom_path(data=plot1_df, aes(step, pred, group=method,
                                 col=method, linetype=method), size=0.7) +
    scale_linetype_manual(values = c("dashed","dashed","dashed", "dashed", "dashed")) +
    scale_color_manual(values=c("black", "orange","#800020", "#FACC5F", "#4169E1")) + 
    scale_y_continuous(breaks=seq(min(plot1_df$pred), max(plot1_df$pred), length.out=2),
                       labels = scientific_10) + 
    scale_x_continuous(breaks = seq(3500, 4500, length.out=2)) +
    geom_vline(xintercept=n_training, linetype="dotted") + 
    xlab('step') +
    ylab("state") +
    theme_classic() + 
...
(zoom_in_plot_list[["4"]])
(zoom_in_plot_list[["8"]])
(zoom_in_plot_list[["12"]])
(zoom_in_plot_list[["16"]])
