...
################ Fig.2 ################
x = seq((n_training+1), N, length.out=n_testing)
y = 1:k
grid <- expand.grid(x=x, y=y)
grid$z <- c(ground_truth) 
truth_with_legend = levelplot(z~x*y,data=grid,
                           col.regions = colorRampPalette(brewer.pal(9, "Blues"))(20),
                           xlab="step", ylab="state", main="Truth", cex=2,
                           cex.main=2)

truth=levelplot(z~x*y,data=grid, 
             col.regions = colorRampPalette(brewer.pal(9, "Blues"))(20),
             xlab="step", ylab="state", main="Truth", 
             cex=3, cex.main=1.5,axis.text=4,
             colorkey = FALSE)


grid$z <- c(pred_ppgp_local_record) 
ppgp_forecast = levelplot(z~x*y,data=grid, 
             col.regions = colorRampPalette(brewer.pal(9, "Blues"))(20),
             xlab="step", main="PP-GP forecast", 
             ylab="",
             cex=2, cex.main=1.5,
             
             colorkey = FALSE)

grid$z <- c(ground_truth-pred_ppgp_local_record) 
pred_error = levelplot(z~x*y,data=grid, 
             col.regions = colorRampPalette(brewer.pal(9, "Blues"))(20),
             xlab="step", ylab="",
             main="Predictive error", 
             cex=2, cex.main=1.5,
             colorkey = FALSE)
